...
test('schema keeps client engagements linked and provisions a private document bucket', () => {
  const schema = read('supabase_schema.sql');
  ...
  assert.match(schema, /INSERT INTO storage\.buckets \(id, name, public\)[\s\S]*VALUES \('documents', 'documents', false\)[\s\S]*ON CONFLICT \(id\) DO UPDATE SET public = false;/);
});
...
test('dashboard pages clear loading when no Supabase user is present', () => {
  ...
  for (const file of dashboardFiles) {
    assert.match(
      ...
      `${file} should clear loading before returning without a user`,
    );
  }
});
