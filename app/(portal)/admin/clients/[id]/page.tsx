...
export default function AdminClientDetailPage() {
    ...
    async function handleDocumentUpload(file: File) {
        ...
        if (uploadError) {
            ...
            setUploadErrorMessage('Upload failed before the document was saved. Please try again.');
            setUploading(false);
            return;
        }
        ...
        if (dbError) {
            ...
            if (rollbackError) {
                console.error('Error rolling back uploaded file:', rollbackError);
            }

            setUploadErrorMessage('Upload failed while saving document metadata. Please try again.');
            setUploading(false);
            return;
        }
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                {activeTab === 'documents' && (
                    <motion.div variants={item} className="p-8 rounded-2xl bg-slate-900 border border-slate-800">
                        ...
                        {uploadErrorMessage && (
                            <div className="mb-6 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-300">
                                {uploadErrorMessage}
                            </div>
                        )}
