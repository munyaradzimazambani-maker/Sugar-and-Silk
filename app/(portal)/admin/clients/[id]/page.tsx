...
export default function AdminClientDetailPage() {
    ...
    const clientId = typeof routeParams.id === 'string' ? routeParams.id : routeParams.id?.[0];
    ...
    async function handleDocumentUpload(file: File) {
        ...
        try {
            ...
            if (dbError) {
                ...
                setUploadError('Upload could not be saved. The file was removed so it can be retried safely.');
                return;
            }
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                {activeTab === 'documents' && (
                    <motion.div variants={item} className="p-8 rounded-2xl bg-slate-900 border border-slate-800">
                        ...
                        {uploadError && (
                            <p className="mb-6 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-300">
                                {uploadError}
                            </p>
                        )}
