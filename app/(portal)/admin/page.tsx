...
export default function AdminClientListPage() {
    ...
    function getClientName(client: any) {
        return client.profiles?.company_name || client.profiles?.full_name || 'Unnamed Client';
    }
    ...
    async function fetchClients() {
        ...
        if (clientData) {
            const sortedClients = [...clientData].sort((a, b) => getClientName(a).localeCompare(getClientName(b)));
            const linkedProfileIds = new Set(sortedClients.map((client) => client.profile_id));
    ...
    const filteredClients = clients.filter(c =>
        getClientName(c).toLowerCase().includes(searchQuery.toLowerCase()) ||
                            ...
                            >
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{getClientName(client)}</p>
                                ...
                                <td className="px-6 py-5 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <button
                                            onClick={() => handleDeleteClient(client.id, getClientName(client))}
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    {availableProfiles.length === 0 && (
                                        <p className="text-xs text-amber-400 mt-2">Create or invite a client user profile before starting an engagement.</p>
                                    )}
                                </div>
