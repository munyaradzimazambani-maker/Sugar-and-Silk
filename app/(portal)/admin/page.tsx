...
export default function AdminClientListPage() {
    ...
    async function fetchClients() {
        ...
        if (data) {
            ...
            setAvailableProfiles((profiles || []).filter((profile: any) => !linkedProfileIds.has(profile.id)));
        }
        setLoading(false);
    }
                                    ...
                                    >
                                        ...
                                        {availableProfiles.map((profile) => (
                                            <option key={profile.id} value={profile.id}>
                                                {profile.company_name || profile.full_name || 'Unnamed client'}
                                            </option>
                                        ))}
                                    </select>
                                    {availableProfiles.length === 0 && (
                                        <p className="mt-2 text-xs text-amber-400">
                                            No unlinked client profiles are available. Create the user profile before starting an engagement.
                                        </p>
                                    )}
                                </div>
