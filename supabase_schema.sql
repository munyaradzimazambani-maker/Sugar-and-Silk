...
-- The application stores portal roles in public.profiles.role. Supabase's JWT
-- "role" claim is the PostgREST database role (usually "authenticated"), so
-- admin policies must check profiles instead.
